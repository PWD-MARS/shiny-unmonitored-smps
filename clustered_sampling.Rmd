...
```{r data gathering, include=FALSE}
   ...
   random_bateches <- dbGetQuery(con," SELECT * FROM fieldwork.tbl_clustered_samples")
   
   #systems in which all smps are NOT available
   system_with_null_smps <- smpbdv_df %>%
     left_join(unmonitored_smp, by = c("system_id","smp_id"), multiple = "all") %>%
     filter(is.na(smp_type)) %>%
     select(system_id) %>%
     distinct() 
   
   #Remove incomplete systems
   unmonitored_smp <- unmonitored_smp %>%
     anti_join(system_with_null_smps, by = "system_id")  
   
   #Remove the previous bateches
   unmonitored_smp <- unmonitored_smp %>%
     anti_join(random_bateches, by = "system_id")
...
#k-means 
# Cluster using kmeans with five clusters
cluster_solution <- kmeans(normalized_cluster[,2:3], centers = 1)
...
#k-means 
# Cluster using kmeans with five clusters
cluster_solution <- kmeans(normalized_cluster[,2:4], centers = 1)
...
#bind all smps
  ...
  db_table <- clustered_systems_table_ad %>%
    group_by(system_id) %>%
    mutate(address = paste(address, collapse = ', '),           date_generated = Sys.Date(),
) %>%
    distinct()
...
```{r sample 0.1 size of the population write to db, include=FALSE}
    #create sample id
    current_sample_id <- dbGetQuery(con,"SELECT max(sample_id) FROM fieldwork.tbl_clustered_samples") %>% pull

  #add the sample id to the table
    new_sample_id <- current_sample_id + 1
   #creating the random samples based on various fraction sizes
   stratified_sample_0.15 <- db_table %>%
     select(-batch_id) %>%
     group_by(sys_modelinputcategory, cluster) %>%
     sample_frac(size= 0.15) %>%
     mutate(sample_id = new_sample_id,
  ...
  dbWriteTable (con, SQL("fieldwork.tbl_clustered_samples"), stratified_sample_0.15, append= TRUE, row.names = FALSE)
